...
export async function POST(request:NextRequest){
    try {
        ...
        const user = await User.findOne({ _id: userId, verifyToken: token, verifyTokenExpiry: { $gt: Date.now() } });
        if (!user) {
            console.log(`user verification failed for userId: ${userId} with token: ${token}`);
